...
# 定义颜色范围的字典
color_dict = {'green': {'Lower': np.array([0, 24, 68]), 'Upper': np.array([92, 136, 140])},
...
if __name__ == '__main__':
    ...
    while flag_open or not flag_video:
        ...
        # 颜色范围的掩膜,获取所需颜色的掩膜
        inRange_hsv_r = cv.inRange(opening_hsv, color_dict['red']['Lower'],
                                   color_dict['red']['Upper'])
        inRange_hsv_b = cv.inRange(opening_hsv, color_dict['blue']['Lower'],
                                   color_dict['blue']['Upper'])
        inRange_hsv_y = cv.inRange(opening_hsv, color_dict['yellow']['Lower'],
                                   color_dict['yellow']['Upper'])
        inRange_hsv_r_2 = cv.inRange(opening_hsv, color_dict['red2']['Lower'], color_dict['red2']['Upper'])
        ...
        for contour in contours:
            ...
            cv.putText(show_img, str(round(area_ratio, 2)), (x + 80, y - 2), cv.FONT_HERSHEY_COMPLEX, 0.5,
                       (0, 0, 255), 1)
            ...
            cv.putText(show_img, color, (x, y - 2), cv.FONT_HERSHEY_COMPLEX, 0.5,
                       (0, 0, 255), 1)
            ...
            cv.putText(show_img, str(len(approx)), (x + 60, y - 2), cv.FONT_HERSHEY_COMPLEX, 0.5,
                       (255, 0, 0), 1)
        ...
        if k == ord('s'):  # 按下s键，进入下面的保存图片操作
            cv.imwrite(r"E:/ultimately/Vision/Things/" + index + ".jpg", img)
            editor()
            ...
            cv.imwrite(r"E:/ultimately/Vision/Things/Log/" + collect_index + str(collect_num) + "_imshow.jpg", show_img)
            cv.imwrite(r"E:/ultimately/Vision/Things/Log/" + collect_index + str(collect_num) + "_inRange_hsv_r.jpg",
                       inRange_hsv_r)
            cv.imwrite(r"E:/ultimately/Vision/Things/Log/" + collect_index + str(collect_num) + "_inRange_hsv_b.jpg",
                       inRange_hsv_b)
            cv.imwrite(r"E:/ultimately/Vision/Things/Log/" + collect_index + str(collect_num) + "_inRange_hsv_y.jpg",
                       inRange_hsv_y)
            flag_collected = 0
        elif not flag_video:
            while True:
