...
def callBackInRange(x):
    ...
    aim = cv.inRange(opening_hsv, np.array([L_H, L_S, L_V]),
                     np.array([H_H, H_S, H_V]))


def editor():
    editor_img = cv.imread(r"E:\ultimately\Vision\Things\red&blue.jpg")
    ...
    aim = cv.inRange(opening_hsv, np.array([0, 0, 0]),
                     np.array([0, 0, 0]))
