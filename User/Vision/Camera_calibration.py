...
savedir = "E:/project/User/Things/camera_data/"
...
objp = np.zeros((check_raw * check_col, 3), np.float32)

# add 2.5 to account for 2.5 cm per square in grid
objp[:, :2] = np.mgrid[0:check_raw, 0:check_col].T.reshape(-1, 2) * 3.85

# Arrays to store object points and image points from all the images.
objpoints = []  # 3d point in real world space
imgpoints = []  # 2d points in image plane.
images = glob.glob("E:/project/User/Things/Camera_calibration/*jpg")
...
for fname in images:
    ...
    ret, corners = cv2.findChessboardCorners(gray, (check_raw, check_col), None)
    ...
    if ret == True:
        ...
        cv2.drawChessboardCorners(img, (check_raw, check_col), corners2, ret)
